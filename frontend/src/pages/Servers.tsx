  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] gap-4">
        ...
        <p className="text-muted-foreground text-sm font-medium">Loading Servers...</p>
      </div>
    )
  }
  ...
  return (
    <div className="w-full space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight mb-1">{t.servers.title}</h1>
          <p className="text-muted-foreground text-sm">{t.servers.subtitle}</p>
        </div>
                ...
                <tr>
                  <td colSpan={6} className="px-6 py-12 text-center text-muted-foreground font-medium">
                    <div className="flex flex-col items-center gap-2">
                      ...
                      <p>No foreign servers found.</p>
                      <p className="text-xs opacity-75">Add a server to get started.</p>
                    </div>
                  </td>
                </tr>
              ) : (
                servers.map((server) => (
                  <tr key={server.id} className="hover:bg-muted/30 transition-colors group">
                    <td className="px-6 py-4 font-semibold">
                      {server.name}
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2 text-xs">
                        <code className="bg-muted px-2 py-1 rounded border border-border text-primary">{server.fingerprint}</code>
                        <button
      ...
      <div className="bg-card w-full max-w-xl max-h-[90vh] rounded-xl border border-border shadow-lg flex flex-col overflow-hidden">
        ...
        <div className="p-6 flex-1 overflow-auto">
          <div className="mb-4 p-3 bg-muted border border-border rounded-md text-sm text-muted-foreground">
            <strong>Foreign Server Installation:</strong> Copy the certificate below. During foreign server installation, you will be prompted to paste this certificate.
          </div>
