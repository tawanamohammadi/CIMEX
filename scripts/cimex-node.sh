#!/bin/bash
# CIMEX Node Installer

set -e

echo "=== CIMEX Node Installer ==="
echo ""
...
INSTALL_DIR="/opt/cimex-node"
echo "Installing to: $INSTALL_DIR"

# Clone from GitHub if needed
if [ -d "$INSTALL_DIR" ] && [ -f "$INSTALL_DIR/docker-compose.yml" ]; then
    echo "CIMEX Node already installed in $INSTALL_DIR"
    cd "$INSTALL_DIR"
else
    echo "Setting up CIMEX Node..."
    mkdir -p "$INSTALL_DIR"
    cd "$INSTALL_DIR"
...
if [ ! -f "Dockerfile" ]; then
    ...
    TEMP_DIR=$(mktemp -d)
    git clone --depth 1 $GIT_BRANCH https://github.com/zZedix/CIMEX.git "$TEMP_DIR" || {
...
# Install CLI
if [ -f "/opt/cimex/cli/cimex-node.py" ]; then
    sudo cp /opt/cimex/cli/cimex-node.py /usr/local/bin/cimex-node
    sudo chmod +x /usr/local/bin/cimex-node
elif [ -f "$INSTALL_DIR/../CIMEX/cli/cimex-node.py" ]; then
    sudo cp "$INSTALL_DIR/../CIMEX/cli/cimex-node.py" /usr/local/bin/cimex-node
    sudo chmod +x /usr/local/bin/cimex-node
else
    ...
    sudo curl -L https://raw.githubusercontent.com/zZedix/CIMEX/${CLI_BRANCH}/cli/cimex-node.py -o /usr/local/bin/cimex-node
    sudo chmod +x /usr/local/bin/cimex-node
fi
...
if docker pull ghcr.io/zzedix/cimex-node:${CIMEX_VERSION} 2>/dev/null; then
...
# Check status
if docker ps | grep -q cimex-node; then
    echo ""
    echo "✅ CIMEX Node installed successfully!"
    echo ""
    echo "Node API: http://localhost:$NODE_API_PORT"
    echo ""
    echo "Check status with: cimex-node status"
    echo ""
else
