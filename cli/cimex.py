...
def get_compose_file():
    ...
    return Path("/opt/cimex") / "docker-compose.yml"
...
def cmd_admin_create(args):
    ...
    try:
        check_result = subprocess.run(
            ["docker", "ps", "-a", "--filter", "name=cimex-panel", "--format", "{{.Names}}"],
            capture_output=True,
            text=True,
            timeout=5
        )
        ...
        if check_result.returncode != 0 or not check_result.stdout.strip():
            print("Container 'cimex-panel' not found.")
            print("\nStarting the panel...")
            compose_file = get_compose_file()
            if not compose_file.exists():
                print(f"Error: docker-compose.yml not found at {compose_file}")
        ...
        while waited < max_wait:
            ...
            if result.returncode == 0 and result.stdout.strip():
                ...
                if "Up" in status and "Restarting" not in status:
                    ...
                    compose_file = get_compose_file()
                    start_result = subprocess.run(
                        ["docker", "compose", "-f", str(compose_file), "start", "cimex-panel"],
                        capture_output=True,
                        text=True
                    )
                    if start_result.returncode == 0:
            ...
            print("Please check container status: docker ps -a | grep cimex-panel")
            sys.exit(1)
...
"""
            ...
            status_proc = subprocess.run(
                ["docker", "ps", "-a", "--filter", "name=cimex-panel", "--format", "table {{.Names}}\\t{{.Status}}"],
                capture_output=True,
                text=True
            )
...
def cmd_status(args):
    ...
    result = subprocess.run(["docker", "ps", "--filter", "name=cimex-panel", "--format", "{{.Status}}"], 
...
def cmd_restart(args):
    ...
    run_docker_compose(["stop", "cimex-panel"])
    run_docker_compose(["rm", "-f", "cimex-panel"])
    run_docker_compose(["up", "-d", "--no-deps", "cimex-panel"])
    
    import time
    time.sleep(2)
    result = subprocess.run(["docker", "ps", "--filter", "name=cimex-panel", "--format", "{{.Status}}"], capture_output=True, text=True)
    if not result.stdout.strip() or "Up" not in result.stdout:
        print("Warning: Panel container may not be running. Check logs with: docker logs cimex-panel")
    
    result = subprocess.run(["docker", "ps", "--filter", "name=cimex-nginx", "--format", "{{.Names}}"], capture_output=True, text=True)
    if result.stdout.strip():
...
def cmd_logs(args):
    ...
    run_docker_compose(["logs"] + follow + ["cimex-panel"])


def cmd_uninstall(args):
    """Uninstall CIMEX Panel - removes everything"""
    print("=" * 60)
    print("⚠️  WARNING: This will completely remove CIMEX Panel!")
    print("=" * 60)
    print("\nThis will remove:")
    print("  - All Docker containers (cimex-panel, cimex-nginx)")
    print("  - All Docker volumes")
    print("  - Installation directory (/opt/cimex)")
    print("  - CLI script (/usr/local/bin/cimex)")
    print("  - Crontab entries related to cimex")
    print("  - Docker images (ghcr.io/zzedix/cimex-panel, ghcr.io/zzedix/cimex-nginx)")
    ...
    try:
        ...
        for container in ["cimex-panel", "cimex-nginx"]:
    ...
    try:
        result = subprocess.run(["docker", "volume", "ls", "-q", "--filter", "name=cimex"], 
                              capture_output=True, text=True)
    ...
    try:
        for image in ["ghcr.io/zzedix/cimex-panel", "ghcr.io/zzedix/cimex-nginx"]:
    ...
    cli_path = Path("/usr/local/bin/cimex")
    if cli_path.exists():
        try:
            cli_path.unlink()
            print("  ✓ Removed /usr/local/bin/cimex")
        except Exception as e:
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            new_lines = [line for line in lines if "cimex" not in line.lower() and "certbot" not in line.lower()]
    ...
    print("\n" + "=" * 60)
    print("✅ CIMEX Panel has been completely uninstalled!")
    print("=" * 60)


def main():
    parser = argparse.ArgumentParser(description="CIMEX Panel CLI")
    ...
    subparsers.add_parser("uninstall", help="Completely remove CIMEX Panel")
