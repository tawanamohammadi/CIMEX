...
def get_compose_file():
    ...
    return Path("/opt/cimex-node") / "docker-compose.yml"
...
def get_env_file():
    ...
    return Path("/opt/cimex-node") / ".env"
...
def run_docker_compose(args, capture_output=False):
    ...
    if not compose_file.exists():
        ...
        print(f"  - /opt/cimex-node/docker-compose.yml")
...
def cmd_status(args):
    ...
    result = subprocess.run(["docker", "ps", "--filter", "name=cimex-node", "--format", "{{.Status}}"], 
...
def cmd_update(args):
    ...
    if (node_dir / ".git").exists():
        ...
        try:
            ...
            compose_url = "https://raw.githubusercontent.com/zZedix/CIMEX/main/node/docker-compose.yml"
...
def cmd_restart(args):
    ...
    run_docker_compose(["stop", "cimex-node"])
    run_docker_compose(["rm", "-f", "cimex-node"])
    result = run_docker_compose(["up", "-d", "--no-deps", "--no-pull", "cimex-node"], capture_output=True)
    if result.returncode != 0 and "--no-pull" in result.stderr:
        run_docker_compose(["up", "-d", "--no-deps", "cimex-node"])
    else:
...
def cmd_logs(args):
    ...
    run_docker_compose(["logs"] + follow + ["cimex-node"])


def cmd_uninstall(args):
    """Uninstall CIMEX Node - removes everything"""
    print("=" * 60)
    print("⚠️  WARNING: This will completely remove CIMEX Node!")
    print("=" * 60)
    print("\nThis will remove:")
    print("  - All Docker containers (cimex-node)")
    print("  - All Docker volumes")
    print("  - Installation directory (/opt/cimex-node)")
    print("  - CLI script (/usr/local/bin/cimex-node)")
    print("  - Docker images (ghcr.io/zzedix/cimex-node)")
    ...
    try:
        ...
        subprocess.run(["docker", "stop", "cimex-node"], capture_output=True, check=False)
        subprocess.run(["docker", "rm", "-f", "cimex-node"], capture_output=True, check=False)
    ...
    try:
        result = subprocess.run(["docker", "volume", "ls", "-q", "--filter", "name=cimex-node"], 
                              capture_output=True, text=True)
    ...
    try:
        subprocess.run(["docker", "rmi", "-f", "ghcr.io/zzedix/cimex-node"], capture_output=True, check=False)
        subprocess.run(["docker", "rmi", "-f", "ghcr.io/zzedix/cimex-node:latest"], capture_output=True, check=False)
        result = subprocess.run(["docker", "images", "--format", "{{.Repository}}:{{.Tag}}", "ghcr.io/zzedix/cimex-node"], 
                              capture_output=True, text=True)
    ...
    install_dirs = [Path("/opt/cimex-node"), Path("/usr/local/node")]
    ...
    cli_path = Path("/usr/local/bin/cimex-node")
    if cli_path.exists():
        try:
            cli_path.unlink()
            print("  ✓ Removed /usr/local/bin/cimex-node")
        except Exception as e:
    ...
    print("\n" + "=" * 60)
    print("✅ CIMEX Node has been completely uninstalled!")
    print("=" * 60)


def main():
    parser = argparse.ArgumentParser(description="CIMEX Node CLI")
    ...
    subparsers.add_parser("uninstall", help="Completely remove CIMEX Node")
